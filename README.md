...
## 原子模式
* 在缺省的 one-by-one 模式中，一个多SQL语句的脚本在执行过程中，当其中任意一条SQL执行失败后，程序下次启动时将会从上次失败的**语句**开始。
* 在原子性执行 atomic 模式中，与 one-by-one 模式的不同之处在于，当其中任意一条SQL执行失败后，程序下次启动时将会从上次失败的**脚本**第一条语句开始。

## 注意事项
* 脚本的版本号不能重复，即便脚本名称不一样。
* 有些数据库不能回滚 DDL 语句，例如 MySQL ，所以